...
def listar_medicos():
    ...
    return jsonify([{"id": m[0], "nome": m[1], "crm": m[2], "especialidade": m[3], "horario": m[4], "genero": m[5]} for m in medicos])
...
def consultar_medico(id):
    ...
    if m:
        ...
        return jsonify({"id": m[0], "nome": m[1], "crm": m[2], "especialidade": m[3], "horario": m[4]})
...
def adicionar_medico():
    ...
    if not all([nome, especialidade, horario]):
        return jsonify({"erro": "Campos 'nome', 'especialidade' e 'horario' são obrigatórios"}), 400
    ...
    conn = sqlite3.connect(DB)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO medicos (nome, crm, especialidade, horario, genero) VALUES (?, ?, ?, ?, ?)",
